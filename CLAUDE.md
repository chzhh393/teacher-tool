...
**当用户提到"记录变更"、"在文档中心记录"时，必须同时完成以下两步：**

1. **更新 `docs/CHANGELOG.md`**：在文件顶部（最新版本之前）新增版本条目
2. **创建开发日志**：在 `docs/dev-logs/YYYY-MM/` 下创建 `YYYY-MM-DD-功能简称.md`
   - 如果当天已有同名文件，用不同的功能简称区分
   - 参考 `docs/dev-logs/TEMPLATE.md` 的格式，包含：本日目标、完成内容、遇到的问题、解决方案、修改文件清单
   - 月份目录不存在时自动创建
