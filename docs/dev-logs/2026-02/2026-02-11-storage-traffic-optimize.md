...
- [x] 备份原始 PNG 到 `~/Desktop/beasts_png_backup_1024x1024.zip`（139MB，从 git 历史恢复，不消耗 CDN 流量）
- [x] 删除静态托管上 164 张旧 PNG 文件（~155MB）
- [x] 删除 180 个历史部署遗留的旧 JS/CSS chunk（~40MB）
- [x] 删除 3 个废弃文件：旧 PWA 图标 `pwa-192x192.png`、`pwa-512x512.png`、旧管理页 `cloud-admin/index.html`
- [x] 静态托管容量：208MB → 6.9MB（释放 96.7%）

### P2.6 - CDN 节点缓存调优 ✅

- [x] `sw.js` 节点缓存：365天 → 2分钟（修复 PWA 更新延迟问题）
- [x] `index.html` 节点缓存：5-7天 → 2分钟（修复部署后延迟生效问题）
- [ ] 浏览器缓存配置：控制台维护中，待后续配置 `sw.js` 和 `index.html` 不缓存
...
| 静态托管旧文件 | 208MB (372 files) | 6.9MB (189 files) | **96.7%** |
...
- 2026-02-11: 静态托管清理旧文件 347 个，容量 208MB → 6.9MB
- 2026-02-11: CDN 节点缓存调优，sw.js/index.html 改为 2 分钟
- 2026-02-11: 部署 v1.5.8
