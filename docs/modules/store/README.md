...
小卖部对子账号的兑换权限通过 `canRedeem` 字段可配置控制：

| 配置 | 效果 |
|------|------|
| `canRedeem: false`（默认） | 子账号可浏览商品和兑换记录，但**不能兑换**，商品卡片显示"仅查看" |
| `canRedeem: true` | 子账号可正常兑换（需在授权班级范围内） |

主账号在创建/编辑子账号时通过"允许兑换小卖部商品"勾选项配置。后端 `TT_shop_redeem` 做双重校验：`role === "sub" && !canRedeem` 时拒绝兑换。
