...
const getClassId = async (db, classId, user) => {
  ...
  const _ = db.command
  const classResult = await db.collection("TT_classes").where(_.or([{ userId: user.userId }, { "data.userId": user.userId }])).limit(1).get()
...
exports.main = async (event = {}) => {
  ...
  const _ = db.command
  const result = await db.collection("TT_students").where(_.or([{ classId }, { "data.classId": classId }])).limit(1000).get()
