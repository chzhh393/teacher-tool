...
const ScoreModal = ({ open, onClose, rules, onSubmit, mode = "add" }: ScoreModalProps) => {
  ...
  return (
    ...
    >
      <div className="flex gap-3 bg-gray-100 p-1 rounded-xl w-fit">
        {[
          ...
          <button
            ...
            className={`rounded-lg px-4 py-1.5 text-sm font-semibold transition-all ${tab === item.key
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 max-h-[60vh] overflow-y-auto p-1">
        {filtered.map((rule) => (
          <button
            type="button"
            key={rule.id}
            onClick={() => onSubmit(rule)}
            className="flex items-center gap-3 card px-4 py-3 text-left border border-transparent hover:border-primary/30"
          >
            <span className="text-2xl">{rule.icon}</span>
            <div>
              <p className="text-sm font-semibold text-text-primary">{rule.name}</p>
              <p className={`text-xs font-semibold ${rule.score > 0 ? "text-success" : "text-danger"}`}>
